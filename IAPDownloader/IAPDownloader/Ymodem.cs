...
namespace IAPDownloader
{
    class Ymodem
    {
        ...
        public bool YmodemUploadFile(string path)
        {
            ...
            const byte NACK = 21;  // Negative ACknowledgement
            ...
            try
            {
                ...
                int count = 0; 
                while (serialPort.ReadByte() != C)
                {
                    if (count++ > 1000)
                    {
                        Console.WriteLine("\r\nCan't begin the transfer.");
                        return false;
                    }
                }
                ...
                /* get NACK (downloader acknowledge the EOT) */
                count = 0;
                while (serialPort.ReadByte() != NACK)
                {
                    if (count++ > 1000)
                    {
                        Console.WriteLine("\r\nCan't recevie negative acknowledgement.");
                        return false;
                    }
                }
